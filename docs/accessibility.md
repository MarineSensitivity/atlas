...
**Version:** 0.10.18 (fix round on 0.10.15/0.10.16) · **Date of this evaluation:** 2026-09-23,
revised after the fix round · **Evaluator:** atlas-8 step 3.
...
| **Scripted keyboard walk, no pointer** | `e2e/keyboard-walk.spec.ts`                      | chromium/webkit/firefox: **18 passed / 0 `fixme`**, every engine. (Before the fix round: 10/5, 9/6, 9/6 of 15 — the round added 3 more tests, for #9/#10/#11, then fixed all 14 open items.) `KNOWN_UNNAMED_STOPS` is empty.                                                                                                                                                           |
...
| 2.1.1 Keyboard                  | Supports              | The whole primary task chain is proven pointer-free on three engines by `keyboard-walk.spec.ts`: select a Program Area from the zones table, read its score, create a place by coordinates, open the report, trigger an export. `places.spec.ts` proves create/rename/remove keyboard-only. The species picker is now a real combobox with its own keyboard model (fix list #8) — `species.smoke.spec.ts` "fix list #8: …". **Exception:** the map canvas — see §3.                                                                                                                                                                                                                                                                                                                                                                            |
| 2.1.2 No Keyboard Trap          | Supports              | Modals are native `<dialog>` + `showModal()`, so Esc always closes; `gallery.spec.ts` "a modal traps focus and returns it to the opener on close". Nothing in the walk ever became unescapable. The inverse defect — Tab _escaping_ an open dialog when its last control is disabled (#2) — is fixed: `keyboard-walk.spec.ts` "A11Y-2: Tab never leaves an open modal, even when its last control is disabled".                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 2.4.1 Bypass Blocks             | Supports              | Two skip links, in DOM/visual/tab order, to the tool rail and the details panel. `tabindex="-1"` on both targets (fix list #6) makes every engine agree on where the next Tab lands — `keyboard-walk.spec.ts` "'Skip to the tools' lands the caret on the tool rail", now green on chromium, webkit AND firefox.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| 3.3.1 Error Identification      | Supports              | Upload and coordinate refusals render verbatim in a `role="alert"` (`CoordinateDialog.svelte:60`, `UploadPanel.svelte:179`) with what/why/fix; the copy itself is gated per-rule by `tests/geo/upload/messages.test.ts`. A bundle-load failure now reports something too (fix list #14, a 3.3.1-adjacent robustness gap rather than the SC itself) — `shell.smoke.spec.ts` "index.html reveals a visible role=alert message when the bundle never runs".                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 2.4.7 Focus Visible                             | Supports                  | A 2 px `--focus-ring` outline on `:focus-visible` throughout, and the roving-tabindex "active" cell is deliberately a _different_, weaker indicator so the two are not confused — `gallery.spec.ts` "the active-but-unfocused cell uses a lighter, dashed indicator, not the strong focus ring". The one control that did not support this, the file-upload input (#3), now does — `keyboard-walk.spec.ts` "A11Y-3: the file-upload control shows a visible focus indicator".                                                                                                                                                                                        |
