<script lang="ts">
  ...
  import { paddedStudyAreaCenter, type ChromePadding } from "../lib/map/camera";
  ...
  function onHelp() {
    // R2 moved "About" from the on-map bottom-left card into the top bar (`data-control="about"`,
    // TopBarActions.svelte) -- this still just points a keyboard/AT user at it until U6's own Help
    // menu lands (a stub, same as before this fix).
    announce("Guided tour and full help arrive in a later phase — see About in the top bar.");
    document.querySelector<HTMLButtonElement>('[data-control="about"]')?.focus();
  }
...
<header class="topbar" data-tour="topbar">
  ...
  <!-- R2 (docs/usability.md §7): About + Feedback + the phone ⋯ menu -- ONE mount line, per this
       ...
       TopBarActions.svelte's header for why this stays a separate component). -->
  <TopBarActions
    {earlyVersion}
