...
// panel's dock+size (or the phone sheet's detent), and the Layers pane's own expanded row.
//
...
// (`g1`, plan D8): `ui=1.<tool>.<dock>.<size>.<detent>.<expandedRow>`. Unknown/malformed (wrong
// version, wrong field count, an out-of-range value, an unrecognized code) is ignored ENTIRELY —
// never a partial apply, matching `codec.ts`'s own "clamp to absent, never throw" rule.
//
...
export const UI_TOKEN_VERSION = "1";
...
export function parseUi(v: string | null | undefined): UiState | null {
  ...
  const [version, toolCode, dockCode, sizeStr, detentCode, rowCode] = parts;
  if (version !== UI_TOKEN_VERSION) return null;
  ...
  if (size < PANEL_SIZE_MIN || size > PANEL_SIZE_MAX) return null;
